 ...
 */
export function showPermissionsDialog(name, initial) {
    return new Promise((resolve) => {
        ...
        <tr>
          ...
          ${perms
            .map(([p]) => `<td><input type="checkbox" class="perm-cb" data-cls="${cls}" data-perm="${p}"></td>`)
            .join("")}
        </tr>`)
            .join("");
        overlay.innerHTML = `
    ...
    `;
        document.body.appendChild(overlay);
        const octalInput = overlay.querySelector("#perm-octal");
        const cbs = Array.from(overlay.querySelectorAll(".perm-cb"));
        const clsShift = { owner: 6, group: 3, other: 0 };
        const permBit = { r: 4, w: 2, x: 1 };
        const modeToGrid = (mode) => {
            for (const cb of cbs) {
        ...
        setTimeout(() => overlay.querySelector("#perm-apply")?.focus(), 10);
    });
}
/**
