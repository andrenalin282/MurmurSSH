...
export class FileBrowser {
    ...
    async handleDownloadMulti() {
        ...
        if (aborted) {
            ...
            this.onStatusMessage?.(t("fileBrowser.downloadCancelledStatus", { done: doneStr, failed: failedStr }), false);
        }
        else if (errors === 0) {
            this.onStatusMessage?.(t("fileBrowser.downloadComplete", { done: t("fileBrowser.downloadedCount", { count: done }), dest: destDir }), false);
        }
        else {
            this.onStatusMessage?.(t("fileBrowser.downloadCompleteErrors", {
    ...
    async moveNamesToDir(names, targetDir) {
        ...
        if (errors === 0) {
            ...
            this.onStatusMessage?.(t("fileBrowser.movedItemsErrors", { count: moved, itemLabel, errors, detail: detailStr }), true);
        }
        this.clearSelection();
        this.setBusy(false);
        await this.refresh();
