...
export class FileBrowser {
    constructor(containerId) {
        ...
        this.busy = false;
        this.cancelConnectCallback = null;
    ...
    setProfile(profileId, defaultPath = "/", localPath = null, protocol = null) {
    ...
    updateLogDom() {
        const panel = this.container.querySelector("#log-panel");
        if (!panel)
            return;
        panel.innerHTML = this.logEntries
            .map((e) => `<div class="log-panel__line log-panel__line--${e.level}">[${e.time}] ${escHtml(e.msg)}</div>`)
            .join("");
     ...
     */
    setConnectingState(connecting, statusText = "", onCancel) {
        this.cancelConnectCallback = onCancel ?? null;
        const statusEl = this.container.querySelector("#connecting-status");
        const textEl = this.container.querySelector("#connecting-status-text");
        const cancelBtn = this.container.querySelector("#cancel-connect-btn");
        if (statusEl)
            statusEl.style.display = connecting ? "" : "none";
        if (textEl)
            textEl.textContent = statusText;
        if (cancelBtn)
            cancelBtn.style.display = connecting ? "" : "none";
    }
    render() {
     ...
     * in a batch prompts.
     */
    async confirmLocalOverwrite(localPath, label) {
        if (this.downloadApplyToAllDecision === "yes")
            return "proceed";
        if (this.downloadApplyToAllDecision === "no")
            return "skip";
        let exists = false;
        try {
    ...
    async handleTerminal() {
        ...
        try {
            ...
            if (needsCopy) {
                const accepted = await showConfirm(t("fileBrowser.copyKeyMsg"), t("fileBrowser.copyKeyTitle"));
                if (accepted) {
    ...
    async handleDownloadFile() {
        ...
        if (!skipOverwriteCheck) {
            ...
            if (decision === "skip") {
                this.status(t("fileBrowser.downloadSkipped"), false);
                return;
            }
            if (decision === "cancel") {
                this.status(t("fileBrowser.downloadCancelledSimple"), false);
                return;
            }
        }
        try {
    ...
    async handleMoveTo() {
        ...
        const targetDir = await showPrompt(t("fileBrowser.moveToTitle", { label }), t("fileBrowser.moveToPlaceholder"), this.currentPath);
        if (!targetDir)
            return;
    ...
    async moveNamesToDir(names, targetDir) {
        ...
        if (errors === 0) {
            ...
            this.status(t("fileBrowser.movedItemsErrors", { count: moved, itemLabel, errors, detail: detailStr }), true);
        }
        this.clearSelection();
        this.setBusy(false);
        await this.refresh();
    ...
    async handleDelete() {
        ...
        if (!entry || !remotePath)
            return;
        if (entry.is_dir) {
            const confirmed = await showConfirm(t("fileBrowser.deleteFolderMsg", { name: entry.name }), t("fileBrowser.deleteFolderTitle"));
            if (!confirmed)
                return;
            try {
                this.setBusy(true);
            ...
            const confirmed = await showConfirm(t("fileBrowser.deleteFileMsg", { name: entry.name }), t("fileBrowser.deleteFileTitle"));
            if (!confirmed)
                return;
            try {
                this.setBusy(true);
    ...
    async handleDeleteMulti(names) {
        if (!this.profileId)
            return;
        const confirmed = await showConfirm(t("fileBrowser.deleteMultiMsg", { count: names.length }), t("fileBrowser.deleteMultiTitle", { count: names.length }));
        if (!confirmed)
            return;
        this.setBusy(true);
        let deleted = 0;
