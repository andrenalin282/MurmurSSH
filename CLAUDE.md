...
- Phase 12.0 (v1.6.0): Profile groups & sorting. Added backward-compatible `group: Option<String>` + `created_at: Option<u64>` to `Profile` (models/profile.rs, serde `skip_serializing_if`). `profile_service::save_profile` stamps `created_at` once (preserved on edit by reading the on-disk value first); `list_profiles` AND `get_profile` backfill `created_at` from the file mtime for legacy profiles (in-memory, never written back) — `get_profile` is the single source of truth so editing a legacy profile keeps its original date instead of resetting to now. New Settings field `profile_sort: Option<String>` ("name"|"created"). Frontend: the profile-selector `<select>` became a collapsible grouped tree — group headers (caret + name + count, click to collapse/expand, per-session state in a `Set<string>`), profile rows (single-click selects via in-place `updateRowSelection`, double-click connects), ungrouped bucket (empty-string key) rendered last; a persisted sort toggle (A–Z | Newest) read from `settings.profile_sort` and saved via read-merge-write. The selector's PUBLIC API and button IDs are unchanged, so main.ts needed NO edits. Profile form gained a `group` text input with a `<datalist>` of existing groups; `show()` became async to prefetch them; save carries `created_at` forward. i18n in all 6 locales (`profiles.ungrouped/sortLabel/sortName(Short)/sortCreated(Short)`, `profileForm.labelGroup/placeholderGroup`). Reviews: per-task spec + quality + Opus final holistic (caught + fixed the legacy-profile created_at reset via get_profile backfill) = SHIP. cargo/clippy/tsc/vite green, 12 lib tests pass.
- Phase 12.1 (v1.6.1): Security/maintenance patch. Bumped **Tauri 2.10.3 → 2.11.1** (`cargo update -p tauri --precise 2.11.1`), which fixes Dependabot alert #7 (medium — Origin Confusion lets remote pages invoke local-only IPC commands; patched in 2.11.1). The upgrade pulled tao 0.34→0.35, wry 0.54→0.55, muda/tray-icon/tauri-build/runtime; `src-tauri/gen/schemas/*.json` regenerated by codegen. Verified clean: cargo build, clippy, 12 lib tests, tsc/vite. Investigated the remaining transitive advisories and **dismissed both as tolerable_risk** (no fix path, unreachable code): glib #2 (`VariantStrIter` unsoundness, fixed in glib 0.20 — unreachable in Tauri 2.x's GTK3 stack, glib stays 0.18.5 after the bump) and rand #6 (low, custom-logger unsoundness we never trigger). Added a **SourceForge mirror** link (https://sourceforge.net/projects/murmurssh/) to README (header + install section) and fixed the stale README languages line (now EN/DE/FR/NL/PL/RU). Website (separate repo MurmurSSHWeb) updated: version 1.3.1→1.6.1, SourceForge in footer + a download button on the Murmur page, and feature/locale copy refreshed for transfer queue, split-pane local browser, permissions column + chmod, profile groups & sorting, keyboard shortcuts, and 6 languages.
- Phase 13.0: Launch-a-profile, parallel windows, desktop shortcuts & group accordion. New `launch_service` (services/launch_service.rs) + `commands/launch.rs`: captures a `--profile <id-or-name>` CLI arg at startup into a `OnceLock` (`capture_launch_arg` first line of `lib.rs run()`), resolves it by exact id then case-insensitive name (`resolve_profile_id`), and exposes `get_launch_profile`/`open_profile_in_new_window`/`create_desktop_shortcut`/`get_desktop_dir` commands. **Parallel connections**: while connected, selecting a *different* profile swaps the Connect button for "Open in new window" (`ProfileSelector.updatePrimaryButtons`, gated on `selectedId !== connectedId` — `setConnected(true, id)` now carries the id), which spawns a second independent process via `current_exe() --profile <id>`. **Auto-connect on launch**: `main.ts` refactored `onConnect` into a hoisted `connectToProfile()`, reused by the `init().then` launch block. **Desktop shortcuts**: right-click a profile row → context menu (reuses `.lb-context-menu`) → "Create desktop shortcut" opens a native Save-As dialog (`@tauri-apps/plugin-dialog` `save()`, defaults to the locale-aware desktop dir via `get_desktop_dir`/`xdg-user-dir DESKTOP`, e.g. ~/Schreibtisch); backend writes the `.desktop`, `chmod 0755`, and marks it trusted (`gio set metadata::trusted true`) so GNOME runs it on double-click; `Exec` prefers `$APPIMAGE` over `current_exe()` so AppImage launchers don't point at an ephemeral mount. **Group accordion**: replaced `collapsedGroups: Set` with a single `expandedGroup: string | null` (one group open at a time, `""`=ungrouped vs `null`=none via `?? null`), persisted in settings as `expanded_profile_group` and restored on launch. Two bugs fixed during testing: orphaned context menu on re-render (hide it at the top of `render()`), and **context-menu items not clickable** (the outside-close `mousedown` listener fired for clicks *inside* the menu, removing it before `click` — guarded with `contains(e.target)`, mirroring local-file-browser). Reviews: per-task spec+quality + final holistic = SHIP. cargo/clippy/tsc/vite green, 21 lib tests (8 new launch_service). Website (MurmurSSHWeb) updated: new "Parallel connections and launchers" home card + `window` icon, profiles card → accordion wording, murmur highlights (+parallel windows, +desktop launchers/--profile).
...
This project is indexed by GitNexus as **MurmurSSH** (1519 symbols, 4161 relationships, 128 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
